// Side Panel script — persistent UI for provider selection and summarize action
// Unlike the popup, this panel stays open across page navigations.
...
function renderPrompts() {
  ...
  allPrompts.forEach((prompt, i) => {
    ...
    option.value = i;
    option.textContent = prompt.length > 65 ? prompt.slice(0, 65) + '…' : prompt;
...
function setupEventListeners() {
  ...
  // Update page info when active tab changes
  chrome.tabs.onActivated.addListener(updatePageInfo);
  chrome.tabs.onUpdated.addListener((_tabId, changeInfo) => {
    if (changeInfo.status === 'complete') updatePageInfo();
  });
}
  ...
  chrome.runtime.sendMessage(
    ...
    (response) => {
      ...
      if (response?.success) {
        ...
        showClipboardHint();
        // Re-enable button after a short delay (panel stays open, unlike popup)
        setTimeout(() => {
          setButtonDisabled(false);
          clearStatus();
        }, 3000);
...
function clearStatus() {
  ...
  el.className = 'status';
  el.textContent = '';
  document.getElementById('clipboardHint').classList.remove('visible');
}
